...
function renderTree() {
    ...
    // Personen nach Generation gruppieren
    const byGeneration = {};
    ...
    const boxHeight = 100;
    const partnerGap = 40;
    const verticalSpacing = 250; // Erhöht für bessere Trennung
    const horizontalSpacing = 120;
    const maxRowWidth = 1000; // Reduziert für bessere Aufteilung

    const positions = new Map();
    const generations = Object.keys(byGeneration).sort((a, b) => a - b);

    // Layout für jede Generation berechnen
    const layout = {};
    ...
    generations.forEach((gen, genIndex) => {
        ...
        // Personen nach Partner-Gruppen und Einzelpersonen gruppieren
        ...
        // Layout für diese Generation berechnen (mit mehreren Zeilen)
        layout[gen] = { rows: [], y: y };
        let currentRow = { items: [], width: 0 };
        
        groupedPersons.forEach(group => {
            const groupWidth = group.length === 2 ? 
                (boxWidth * 2 + partnerGap + horizontalSpacing) : 
                (boxWidth + horizontalSpacing);
                
            // Wenn diese Gruppe nicht in die aktuelle Zeile passt, neue Zeile beginnen
            if (currentRow.width + groupWidth > maxRowWidth && currentRow.items.length > 0) {
                layout[gen].rows.push(currentRow);
                currentRow = { items: [], width: 0 };
            }
            
            currentRow.items.push(group);
            currentRow.width += groupWidth;
        });
        ...
        // Positionen für jede Gruppe berechnen
        let rowY = y;
        layout[gen].rows.forEach(row => {
            ...
            // Nächste Zeile weiter unten
            rowY += verticalSpacing / 1.2; // Reduzierter Abstand zwischen Zeilen
        });
        
        // Gesamthöhe für diese Generation aktualisieren
        const genHeight = (layout[gen].rows.length * verticalSpacing / 1.2) - verticalSpacing / 3;
        totalSvgHeight = Math.max(totalSvgHeight, y + genHeight + boxHeight + 100);
        
        // Gesamtbreite für diese Generation aktualisieren
        let maxGenWidth = 0;
        layout[gen].rows.forEach(row => {
            maxGenWidth = Math.max(maxGenWidth, row.width);
        });
        totalSvgWidth = Math.max(totalSvgWidth, maxGenWidth + 200);
    });
    ...
    // Generationsbeschriftungen hinzufügen
    generations.forEach((gen, genIndex) => {
    ...
    // Schattenfilter für Hover-Effekt
