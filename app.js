...
function renderTable() {
    ...
    const genClassMap = {
        1: "gen1-color", 2: "gen2-color", 3: "gen3-color",
        4: "gen4-color", 5: "gen5-color", 6: "gen6-color", 7: "gen7-color"
    };
    ...
    for (const p of people) {
        ...
        const gen = p.Gen || 1;
        const genClass = genClassMap[gen] || "";
        if (genClass) {
            tr.classList.add(genClass);
        }
...
function renderTree() {
    ...
    const genColors = {
        1: "var(--gen1-color)", 2: "var(--gen2-color)", 3: "var(--gen3-color)",
        4: "var(--gen4-color)", 5: "var(--gen5-color)", 6: "var(--gen6-color)", 7: "var(--gen7-color)"
    };
    ...
    sortedPeople.forEach(person => {
        ...
        const gen = person.Gen || 1;
        const color = genColors[gen] || "#f9fafb"; // Use CSS variable here for display

        const personGroup = document.createElementNS(svgNS, "g");
        personGroup.setAttribute("class", `node gen${gen}-color`); // Add gen-specific class for print
        ...
        rect.setAttribute("fill", color); // Fill with CSS variable
